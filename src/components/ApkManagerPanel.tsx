...
export const ApkManagerPanel: React.FC<ApkManagerPanelProps> = ({
  ...
  const handleInstallToDevice = async () => {
    ...
    if (!selectedDevice) {
      setInstallStatus('Error: No target device connected. Please connect a physical device or pair via Wireless ADB.');
      return;
    }

    setIsInstalling(true);
    setInstallStatus(`Installing ${selectedApk.fileName} to ${selectedDevice.name} (${selectedDevice.id})...`);

    try {
      const res = await AdbService.installApk(selectedDevice.id, selectedApk.fullPath, projectName);
      if (res.success) {
        setInstallStatus(`Successfully installed ${selectedApk.fileName} on device ${selectedDevice.name}!`);
      } else {
        setInstallStatus(`Installation failed: ${res.reason || res.error || res.status}`);
      }
    } catch (err: any) {
      setInstallStatus(`Install error: ${err.message}`);
    } finally {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Android APK: ${apk.fileName}`,
          text: `Download Android Application Package (${apk.sizeFormatted})`,
    ...
    <div className="flex-1 flex flex-col h-full bg-[#18191c] text-[#bcbec4] select-none text-xs font-mono overflow-hidden">
      ...
      <div className="bg-[#1e1f22] border-b border-[#2b2d30] px-3 py-2 flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-bold text-white text-xs">APK Manager</span>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-[#2b2d30]/50">
            {apks.length === 0 ? (
              <div className="p-6 text-center text-gray-500 flex flex-col items-center">
                ...
                <div className="text-white font-medium mb-1">NO APK AVAILABLE</div>
                <p className="text-[11px] text-gray-400 max-w-[220px] mb-4">
                  No APK output detected in build output directories for project "{projectName}".
                </p>
                <button
                  onClick={() => onBuildApk('debug')}
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col justify-between">
          {selectedApk ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <div className="bg-[#1e1f22] border border-[#2b2d30] rounded-lg p-3">
                  <div className="text-[10px] text-gray-400 uppercase font-semibold">Package Name</div>
                  <div className="text-white font-bold text-xs mt-1 truncate">
                    {selectedApk.packageName || 'UNKNOWN'}
                  </div>
                </div>
                <div className="bg-[#1e1f22] border border-[#2b2d30] rounded-lg p-3">
                  <div className="text-[10px] text-gray-400 uppercase font-semibold">Version Name & Code</div>
                  <div className="text-white font-bold text-xs mt-1">
                    {selectedApk.versionName !== 'UNKNOWN' ? `v${selectedApk.versionName}` : 'UNKNOWN'} (Build #{selectedApk.versionCode})
                  </div>
                </div>
                <div className="bg-[#1e1f22] border border-[#2b2d30] rounded-lg p-3">
                  <div className="text-[10px] text-gray-400 uppercase font-semibold">File Size</div>
                  <div className="text-[#3ddc84] font-bold text-xs mt-1">
                    {selectedApk.sizeFormatted} ({selectedApk.sizeBytes.toLocaleString()} bytes)
                  </div>
                </div>
                <div className="bg-[#1e1f22] border border-[#2b2d30] rounded-lg p-3">
                  <div className="text-[10px] text-gray-400 uppercase font-semibold">Build Variant</div>
                  <div className="text-white font-bold text-xs mt-1 uppercase">{selectedApk.buildVariant}</div>
                </div>
                <div className="bg-[#1e1f22] border border-[#2b2d30] rounded-lg p-3">
                  <div className="text-[10px] text-gray-400 uppercase font-semibold">SDK Targets</div>
                  <div className="text-white font-bold text-xs mt-1">
                    Min SDK: {selectedApk.validationDetails?.minSdk || 'UNKNOWN'} • Target SDK: {selectedApk.validationDetails?.targetSdk || 'UNKNOWN'}
                  </div>
                </div>
                <div className="bg-[#1e1f22] border border-[#2b2d30] rounded-lg p-3">
                  <div className="text-[10px] text-gray-400 uppercase font-semibold">Validation Method</div>
                  <div className="text-white font-mono text-[11px] mt-1 truncate">
                    {selectedApk.validationDetails?.validationMethod || 'Basic Archive Check'}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#1e1f22] border border-[#2b2d30] rounded-lg p-3.5 space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2 pt-1 text-[11px]">
                  <div className="flex items-center gap-1 text-gray-400">
                    ...
                    <span>ZIP Header: {selectedApk.validationDetails?.hasZipMagic ? 'Present' : 'Missing'}</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-400">
                    <span className={`w-1.5 h-1.5 rounded-full ${selectedApk.validationDetails?.hasManifest ? 'bg-[#3ddc84]' : 'bg-amber-400'}`} />
                    <span>Manifest: {selectedApk.validationDetails?.hasManifest ? 'Found' : 'Unchecked'}</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-400">
                    <span className={`w-1.5 h-1.5 rounded-full ${selectedApk.validationDetails?.apksignerVerified ? 'bg-[#3ddc84]' : 'bg-amber-400'}`} />
                    <span>apksigner: {selectedApk.validationDetails?.apksignerVerified ? 'Passed' : 'Unavailable'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#1e1f22] border border-[#2b2d30] rounded-lg p-4 space-y-3">
                <h4 className="text-white text-xs font-bold uppercase tracking-wider flex items-center gap-2">
                  ...
                  <span>Install to Connected Device (ADB)</span>
                </h4>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#18191c] p-3 rounded border border-[#2b2d30]">
                  <div>
                    <div className="text-gray-400 text-[11px]">Selected Target Device:</div>
                    <div className="font-semibold text-white text-xs mt-0.5 flex items-center gap-1.5">
                      {selectedDevice ? (
                        ...
                        <span className="text-amber-400 flex items-center gap-1">
                          ...
                          <span>No Device Connected via ADB</span>
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    >
                      ...
                      <span>{isInstalling ? 'Installing via ADB...' : 'Install on Device'}</span>
                    </button>
                  </div>
                </div>
                ...
                {installStatus && (
                  <div className={`p-2.5 rounded border text-[11px] font-mono ${
                    installStatus.includes('Success') 
                      ? 'bg-[#3ddc84]/10 border-[#3ddc84]/30 text-[#3ddc84]' 
                      : installStatus.includes('Error') || installStatus.includes('failed')
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-gray-500">
              ...
              <div className="text-white font-medium text-sm mb-1">No APK Package Selected</div>
              <p className="text-xs text-gray-400 max-w-sm mb-4">
                Select an APK from the left list or compile a fresh package using Gradle build tasks.
              </p>
              <button
                onClick={() => onBuildApk('debug')}
